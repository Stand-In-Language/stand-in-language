...
lamF x = (`PairFrag` EnvFrag) <$> deferF x

clamF :: BreakState' a -> BreakState' a
clamF x = (`PairFrag` ZeroFrag) <$> deferF x
...
instance TelomareLike LExpr where
  fromTelomare = \case
    Zero -> LZero
    Pair a b -> LPair (fromTelomare a) (fromTelomare b)
    Env -> LEnv
    SetEnv x -> LSetEnv $ fromTelomare x
    Defer x -> LDefer $ fromTelomare x
    PLeft x -> LLeft $ fromTelomare x
    PRight x -> LRight $ fromTelomare x
    Trace -> LTrace
    -- Gate l r -> LDefer (LSetEnv (LPair LGate (LPair (fromTelomare l) (fromTelomare r))))
    Gate l r -> LGate (fromTelomare l) (fromTelomare r)
  toTelomare = \case
    -- LDefer (LSetEnv (LPair LGate (LPair l r))) -> Gate <$> toTelomare l <*> toTelomare r
    LGate l r -> Gate <$> toTelomare l <*> toTelomare r
    LZero -> pure Zero
    LPair a b -> Pair <$> toTelomare a <*> toTelomare b
    LEnv -> pure Env
    LSetEnv x -> SetEnv <$> toTelomare x
    LDefer x -> Defer <$> toTelomare x
    LLeft x -> PLeft <$> toTelomare x
    LRight x -> PRight <$> toTelomare x
    LTrace -> pure Trace
    -- LGate -> Nothing
    LCloj _ _ -> Nothing
